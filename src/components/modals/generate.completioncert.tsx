...
function GenerateCompletionCertification({
  id,
  initialvalue,
  callback,
}: {
  ...
  return (
    <>
      ...
      <Modal size={'3xl'} isOpen={isOpenEdit} onClose={() => {postData(); callback(); onCloseEdit()}}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Generate a Compelition Certification</ModalHeader>
  ...
  function postData() {
    setIsloading(true);
    apiProvider
      .uploadDoc({ name: 'completioncert', id: id, content: value })
      .then(data => {
        setIsloading(false);
      })
      .catch(e => {
...
export default GenerateCompletionCertification;
